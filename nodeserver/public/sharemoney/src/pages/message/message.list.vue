        ...
        .panel_max-width.mx-auto
          ...
          el-card.box-card(v-if="!isEditorOpen")
            div(slot="header" class="clearfix")
              span( style="line-height: 36px;") 消息记录 <br/>
            ...
            ul(v-loading.body="isListLoading")
              li.m1.flex.border.justify-between.item(v-for="i in list", data-index="i",  @click="getDetail(i.videoId)")
                ...
                .justify-around.self-center.flex1.p2
                  span(v-if="i.teamMessage.msgSenderId == i.teamMessage.followId.split('-')[1]")
                    span.keyword.keyword-primary 医生回复
                  span {{ i.senderName || '未知'}}
                    ...
                    | {{ i.receiverName || '未知'}}：
                    span(v-if="i.teamMessage.msgText") {{i.teamMessage.msgText | removeTag }}
                    span(v-if="i.teamMessage.msgAudio" ) [语音消息]
                      .p2
                        audio.border( style="display: block !important;height:35px;width:90px;", controls="controls")
                          source( :src="i.teamMessage.msgAudio", type="audio/ogg")
                    span(v-if="i.teamMessage.msgImage" ) [图片消息] <br/>
                      img.p2( :src="i.teamMessage.msgImage+'!110'", style="max-width:100%;max-height:110px;")
                  .gray.small.text-right {{i.teamMessage.createTime | goodTime}}
...
export default {
  ...
  methods: {
    getList(targetPageNo){ //获取消息列表
      //this.currentDocId &&
        this.$set( this.$data, 'isListLoading', true )
        ajax('ddyy.team.message.list.group', {
