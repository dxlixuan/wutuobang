        ...
        .panel_max-width.mx-auto
          ...
          el-card.box-card(v-if="!isEditorOpen")
            div(slot="header" class="clearfix")
              span( style="line-height: 36px;") 微课管理
              el-button( style="float: right;", type="primary", @click="getDetail()") 添 加
            ul(v-loading.body="")
              li.m1.flex.border.justify-between.justify-center.hand.item(v-for="i in list", data-index="i",  @click="getDetail(i.videoId)")
                img.m2.border-box.bg(style="width:128px;height:72px", v-bind:src="i.videoLogo+'!110'")
                .justify-around.self-center.flex1 {{i.videoTitle|| '视频标题'}}
                  .gray.small {{ i.docName || '暂无' }}&nbsp;{{ i.hosName || '暂无医院' }}&nbsp;
                    span(style="display:inline-block")
                      span {{ i.deptName || '暂无科室' }}&nbsp;
                      span {{ i.docTitle }}
                  .gray.small 观看次数 {{ i.playCount?i.playCount:0 }}
                .self-center.p4 
                  i.el-icon-arrow-right.gray
              .text-center.gray.p4(v-if="list && list.length==0 ") 暂无视频
          ...
          el-card.box-card(v-if="isEditorOpen")
            ...
            el-form( ref="form", :model="form", label-width="120px" )
              ...
              el-form-item( label="观看次数" )
                .gray {{ form.playCount?form.playCount:0 }}
              ...
              el-form-item( label="价格(心意)" )
                el-input( v-model="form.videoFee")
              ...
              el-form-item( label="简介图片" )
                el-upload.avatar-uploader( v-bind:action="uploadInfo.host", :data="uploadInfo.basePostData", :show-file-list="false", :on-success="upVideoImgSuccess", :before-upload="beforePicUpload")
                  img.avatar( style="width:256px ; height:144px;", v-if="form_videoImg && form_videoImg[0]", :src="form_videoImg[0].url")
                  i.el-icon-plus.avatar-uploader-icon(v-else)
...
export default {
  ...
  methods: {
    getData(){ //获取视频列表
      ajax('ddyy.team.micro.video.list'/*,{groupId: this.currentGroupId, token: this.$store.state.userInfo.token}*/)
        .then( res =>{
